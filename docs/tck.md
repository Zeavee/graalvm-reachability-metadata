...
| `nativeTestCompileLayered` | Build the native image used by native tests with `LayerUse` pointing at the shared base layer. |
| `testLayered` | Run the native tests with `LayerUse` pointing at the shared base layer. |
...
`native-image --version`, `java -version`, OS/architecture, native-image mode,
base-layer module set (`java.base`, `java.management`, `java.sql`,
...
`jdk.sctp`, and `jdk.localedata`), and build arguments,
so a stale or mismatched layer is rejected before any per-coordinate native
image is built.

`testLayered` can run a coordinate batch in collecting mode with
